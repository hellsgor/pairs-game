export function gameMove(event) {
  ...
  const allFronts = Array.from(document.getElementsByClassName('card__exactly-card_front'));
  ...
  console.log('before', visibleFronts);

  if (visibleFronts.length < 2) {
    visibleFronts.push(event.target.closest('.card__slot').querySelector('.card__exactly-card_front'));
  }

  console.log('after', visibleFronts);
  ...
  if (visibleFronts.length === 2) {
    ...
    const firstCardNumber = +visibleFronts[0].querySelector('.card__number').textContent;
    const secondCardNumber = +visibleFronts[1].querySelector('.card__number').textContent;
    ...
    window.setTimeout(() => {
      if (allFronts.every((front) => front.classList.contains('card__slot_guessed'))) {
        console.log('you win');
      } else {
        allFronts.forEach((front) => {
          const slot = front.closest('.card__slot');
